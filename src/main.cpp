...
uint256 hashGenesisBlock("0x000009c80d1a2fda7b9fe334ca1f4dbc8c4475e10983c50f6f64e0dcfb107437");

static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // texascoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "texascoin Signed Message:\n";
...
{
    // texascoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nBits, int nHeight, int64 nFees)
{
    ...
    else if(nHeight > 10000 && nHeight <= 110000)
    {
       nSubsidy = 15 * COIN;
    }
    else if(nHeight > 110000 && nHeight <= 1110000)
    {
       nSubsidy = 4 * COIN;
    }
    else if(nHeight > 1110000 && nHeight <= 11110000)
    {
       nSubsidy = 2 * COIN;
    }
...
static const int64 nTargetTimespan = 1 * 60 * 60; // Texascoin: 1 hour
static const int64 nTargetSpacing = 1 * 60; // Texascoin: 1 Minute
...
unsigned int static GetNextWorkRequired_V1(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // texascoin: This fixes an issue where a 51% attack can change difficulty at will.
...
unsigned int static DarkGravityWave(const CBlockIndex* pindexLast, const CBlockHeader *pblock) {
    /* current difficulty formula, texascoin - DarkGravity, written by Evan Duffield - evan@texascoin.io */
    const CBlockIndex *BlockLastSolved = pindexLast;
    const CBlockIndex *BlockReading = pindexLast;
    const CBlockHeader *BlockCreating = pblock;
    BlockCreating = BlockCreating;
...
unsigned int static DarkGravityWave2(const CBlockIndex* pindexLast, const CBlockHeader *pblock) {
    /* current difficulty formula, texascoin - DarkGravity v2, written by Evan Duffield - evan@texascoin.io */
    const CBlockIndex *BlockLastSolved = pindexLast;
    const CBlockIndex *BlockReading = pindexLast;
    const CBlockHeader *BlockCreating = pblock;
    BlockCreating = BlockCreating;
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // texascoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
{
    // texascoin: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x000001c853d57f8bf77bd5fa2064de04500ce51bbb429e0c11d70d336df4ef17");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "Texas Coin";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1403912951;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2864650;

        if (fTestNet)
        {
            block.nTime    = 1403824928;
            block.nNonce   = 366010;
        }
        ...
        assert(block.hashMerkleRoot == uint256("0xc0f9b01fb742e28cdf5f06d87b4479a62cfcd01633729bff68fef8df6a4cf3ef"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xa3, 0xd5, 0xc2, 0xf9 }; // texascoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("texascoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("texascoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static texascoinMiner(CWallet *pwallet)
{
    printf("texascoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("texascoin-miner");
    ...
    try { while (true) {
        ...
        printf("Running texascoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("texascoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&texascoinMiner, pwallet));
}
